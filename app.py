...
# --- Массивы скинов (cup_skins, plate_skins, car_skins, boot_skins, lightning_skins) ---
cup_skins = [
...
def get_user_from_db(user_id):
    ...
    if user is None:
        db.execute('INSERT INTO users (user_id, balance, vc_balance, last_active) VALUES (?, ?, ?, ?)',
        ...
        inventory = [json.loads(row['item']) for row in db.execute('SELECT item FROM inventory WHERE user_id=?', (user_id,))]
...
def save_user_to_db(user_id, balance, inventory, last_active, vc_balance):
    db = get_db()
    user = db.execute('SELECT * FROM users WHERE user_id=?', (user_id,)).fetchone()
    if user is None:
        db.execute('INSERT INTO users (user_id, balance, vc_balance, last_active) VALUES (?, ?, ?, ?)',
                   (user_id, balance, vc_balance, last_active))
    else:
        db.execute('UPDATE users SET balance=?, vc_balance=?, last_active=? WHERE user_id=?',
                   (balance, vc_balance, last_active, user_id))
    db.execute('DELETE FROM inventory WHERE user_id=?', (user_id,))
    for item in inventory:
        db.execute('INSERT INTO inventory (user_id, item) VALUES (?, ?)', (user_id, json.dumps(item)))
    db.commit()
...
def save_data():
    ...
    save_user_to_db(user_id, balance, inventory, last_active, vc_balance)
...
def top_users():
    db = get_db()
    cur = db.execute("SELECT user_id, balance, vc_balance FROM users ORDER BY balance DESC LIMIT 10")
    top = [
...
def save_db():
    # Сохраняем дамп базы в SQL-файл
    try:
        db = get_db()
...
def restore_db():
    # Восстанавливаем базу из SQL-файла
    try:
