...
# ГАРАНТИРОВАННАЯ инициализация базы внутри application context!
...
def sell_item():
    """ Продаём предмет из инвентаря, начисляем VC """
    data = request.json
    user_id = data['user_id']
    item_idx = data['item_idx']  # индекс предмета в инвентаре
    user_data = get_user_from_db(user_id)
    try:
    ...
    db = get_db()
    # Находим в базе конкретный unsold предмет с таким же json
