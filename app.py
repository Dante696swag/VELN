...
SQL_DUMP_FILE = os.path.join(BASE_DIR, 'users_dump.sql')
...
def get_db():
    ...
    if db is None:
        ...
        # Добавим level и exp в таблицу users
        db.execute('''
...
def get_user_from_db(user_id):
    ...
    if user is None:
        db.execute('INSERT INTO users (user_id, balance, vc_balance, last_active, username, first_name, last_name, photo_url, level, exp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                   (user_id, 0, 0, int(time.time()), "", "", "", "", 1, 0))
        db.commit()
        inventory = []
        ...
        exp = user['exp'] if 'exp' in user.keys() else 0
    return {
        "balance": balance,
...
def save_user_to_db(user_id, balance, inventory, last_active, vc_balance, username="", first_name="", last_name="", photo_url="", level=1, exp=0):
    ...
    if user is None:
        db.execute('INSERT INTO users (user_id, balance, vc_balance, last_active, username, first_name, last_name, photo_url, level, exp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                   (user_id, balance, vc_balance, last_active, username, first_name, last_name, photo_url, level, exp))
    else:
        db.execute('UPDATE users SET balance=?, vc_balance=?, last_active=?, username=?, first_name=?, last_name=?, photo_url=?, level=?, exp=? WHERE user_id=?',
                   (balance, vc_balance, last_active, username, first_name, last_name, photo_url, level, exp, user_id))
...
def top_users():
    db = get_db()
    cur = db.execute("SELECT user_id, balance, vc_balance, username, first_name, last_name, photo_url, level, exp FROM users")
...
def restore_db():
    try:
        ...
        db.commit()
        db.close()
        # Таблицы будут созданы автоматически при следующем get_db()
        return jsonify({"success": True, "message": "Database restored from users_dump.sql"})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})

if __name__ == '__main__':
    # Для инициализации базы вручную: python3 app.py init_db
    import sys
