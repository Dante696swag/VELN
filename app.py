...
# Абсолютный путь к базе (чтобы не было проблем с рабочей директорией)
...
# Создание БД и таблиц при каждом запуске процесса
init_db()
...
def restore_db():
    try:
        ...
        db.commit()
        db.close()
        # После восстановления -- чтобы таблицы были гарантированно созданы
        init_db()
