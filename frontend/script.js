// === VELN script.js: полный фикс баланса, топа, сохранений, предупреждения о монетах ===
...
async function showTopTab(tab = "balance") {
    try {
        ...
        if (!Array.isArray(data)) {
            throw new Error("Топ не массив");
        }
        if (tab === "vc") {
            let sorted = [...data].sort((a, b) => (b.vcBalance || 0) - (a.vcBalance || 0));
            html = sorted.slice(0, 10).map((user, idx) => `
              <li>
                ...
                <span class="top-vc">${user.vcBalance || 0} VC</span>
        ...
        document.getElementById("top-list-balance").innerHTML = `<li>⚠️ Не удалось получить топ игроков</li>`;
        document.getElementById("top-list-vc").innerHTML = `<li>⚠️ Не удалось получить топ игроков</li>`;
