// === VELN script.js: современный, фиксированный, с бургер-меню, красивой кнопкой, AR только в меню, плавная работа вкладок ===
...
// --- Toggle logic for inventory/top tabs ---
let openedTab = null;

// --- DOMContentLoaded ---
document.addEventListener("DOMContentLoaded", () => {
    // Мобильная прокрутка: убираем ограничения высоты
    ...
    document.querySelectorAll('.veln-nav-btn').forEach(btn => {
        btn.onclick = () => {
            ...
            if (openedTab === tab) {
                ...
                // Открываем новую, закрываем все остальные
                document.querySelectorAll(".tab-content").forEach(t => t.classList.add("hidden"));
    ...
    document.getElementById("menu-btn").onclick = function() {
        ...
        // Блокируем прокрутку основного контента на мобильных при открытом меню
...
function setupEventListeners() {
    ...
    // Обеспечить прокрутку при смене вкладки (особенно на мобильных)
    document.querySelectorAll('.veln-nav-btn,.top-tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            setTimeout(() => {
                window.scrollTo({top: 0, behavior: "smooth"});
            }, 10);
