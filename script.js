// ========== КОНФИГ ==========
const ITEMS = [
  // ... (оставь свой массив ITEMS без изменений)
];
...
// ========== API ==========
// !!! На время теста юзаем фиксированный user_id, чтобы данные всегда были одни и те же !!!
const USER_ID = "dante-test-1"; // Для продакшена подключи Telegram WebApp как было
  ...
  panels.inventory.querySelectorAll('.grok-item-sell').forEach(btn=>{
    btn.onclick = async function() {
      ...
      if(res.success) {
        ...
        updateBalances();
        renderInventory();
        await apiSaveUser(); // Сохраняем после продажи!
      }
    };
  });
}
...
function spinRoulette(items, winnerIdx, itemWidth, visible) {
  ...
  setTimeout(async ()=>{
    ...
    await apiSaveUser(); // Сохраняем после выигрыша!
    updateBalances();
    renderInventory();
...
function renderAddPoint() {
  ...
  document.getElementById('btn-addpoint').onclick = async () => {
    setInterval(async ()=>{
      ...
      updateBalances();
      await apiSaveUser(); // Сохраняем каждую секунду чтобы не терять очки!
    }, 1000);
  };
}
    ...
    document.getElementById('top-players-list').innerHTML = list
      ...
      .map((u,i)=>`
        <div class="grok-leader-row">
          ...
          <span style="font-size:.85em;margin-left:.6ем;color:var(--grok-neon2);">LVL${u.level||1}</span>
        </div>
      `).join('');
  });
}
...
// Можно убрать автообновление, если не нужно видеть чужие балансы в реальном времени
// setInterval(async()=>{ await loadUser(); }, 30000);